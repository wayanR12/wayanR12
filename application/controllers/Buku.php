...
class Buku extends CI_Controller {
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == true) {
				if($this->Buku_model->update($this->input->post('id')) == true){
					$this->session->set_flashdata('announce', 'Berhasil menyimpan data buku');
					redirect('buku/edit?idtf='.$this->input->post('id'));
				}else{
					$this->session->set_flashdata('announce', 'Gagal menyimpan data buku');
					redirect('buku/edit?idtf='.$this->input->post('id'));
				}
			} else {
				$this->session->set_flashdata('announce', validation_errors());
	...
	public function delete(){
		...
		if($this->Buku_model->delete($id) == true){
			$this->session->set_flashdata('announce', 'Berhasil menghapus data buku');
			redirect('buku');
		}else{
			$this->session->set_flashdata('announce', 'Gagal menghapus data buku');
			redirect('buku');
		}
	}
