...
class Anggota extends CI_Controller {
		...
		if($this->input->post('submit')){
			...
			//if ($this->form_validation->run() == true) {
				if($this->Anggota_model->update($this->input->post('id')) == true){
					$this->session->set_flashdata('announce', 'Berhasil menyimpan data siswa');
					redirect('anggota/edit?idtf='.$this->input->post('id'));
				}else{
					$this->session->set_flashdata('announce', 'Gagal menyimpan data siswa');
					redirect('anggota/edit?idtf='.$this->input->post('id'));
				}
			/* } else {
				$this->session->set_flashdata('announce', validation_errors());
	...
	public function delete(){
		...
		if($this->Anggota_model->delete($id) == true){
			$this->session->set_flashdata('announce', 'Berhasil menghapus data siswa');
			redirect('anggota');
		}else{
			$this->session->set_flashdata('announce', 'Gagal menghapus data siswa');
			redirect('anggota');
		}
	}
}
